...
const server = http.createServer((req,res) => {
  ...
  if (req.method === 'POST') {
    ...
    if(url.pathname === '/api/import-corrections') {const user=authenticated(req);if(!user)return json(res,401,{error:'Faça login para importar produção.'});readBody(req,16*1024*1024).then(async body=>{try{const input=JSON.parse(body),bytes=Buffer.from(String(input.data||''),'base64');if(!bytes.length||bytes.length>10*1024*1024||bytes[0]!==80||bytes[1]!==75)throw Error('Selecione um arquivo Excel .xlsx de até 10 MB.');const result=await parseCorrections(bytes,await cloud.read());if(input.confirm){const all=result.rows.filter(row=>row.status==='matched'||row.status==='new'),production=result.kind==='production',offset=Math.max(0,Number(input.offset)||0),limit=Math.min(300,Math.max(1,Number(input.limit)||200)),rows=production?all.slice(offset,offset+limit):all;await (production?cloud.importProduction(rows,user):cloud.importCorrections(rows,user));result.imported=rows.length;result.totalImportable=all.length;result.nextOffset=production?offset+rows.length:all.length;result.complete=!production||result.nextOffset>=all.length;}json(res,200,result);}catch(e){json(res,400,{error:e.message});}}).catch(e=>json(res,400,{error:e.message}));return;}
