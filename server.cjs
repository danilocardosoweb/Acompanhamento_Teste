...
async function queueAutomaticWhatsappSummary(){
  ...
  for(const group of groups){const id=crypto.randomUUID(),command={id,type:'summary',groupId:group.whatsapp_group_id,groupName:group.name,summary,createdAt,automatic:true};fs.writeFileSync(path.join(whatsappBotDir,`whatsapp-summary-command-${id}.json`),JSON.stringify(command));}
