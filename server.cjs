...
const server = http.createServer((req,res) => {
  ...
  if (req.method === 'POST') {
    ...
    if(url.pathname === '/api/correction') {const user=authenticated(req);if(!user)return json(res,401,{error:'Faça login para registrar a correção.'});readBody(req,36*1024*1024).then(async body=>{try{const input=JSON.parse(body);if(!/^[0-9a-f-]{36}$/.test(String(input.id||''))||String(input.text||'').length>10000)throw Error('Dados inválidos.');await validateLocation(input.id,input.location);await cloud.saveCorrection({id:input.id,text:String(input.text||''),user,file:correctionFile(input.file)});if(input.location)await cloud.saveLocation({location:input.location,correctionId:input.id,user});json(res,200,{ok:true});}catch(e){json(res,400,{error:e.message});}}).catch(e=>json(res,400,{error:e.message}));return;}
