 ...
 async function processQueue(){
  ...
  for(const row of rows||[]){
   ...
   try{
    ...
    // marcar como erro uma mensagem que já foi entregue ao WhatsApp.
    let sentError=null;
    for(let attempt=0;attempt<3;attempt++){
     const result=await supabase.from('quality_whatsapp_notifications').update(payload).eq('id',row.id);
     sentError=result.error||null;
     if(!sentError)break;
     await new Promise(resolve=>setTimeout(resolve,250*(attempt+1)));
    }
    if(sentError){
     log.error('Mensagem enviada, mas a confirmação da fila falhou; não será reenfileirada imediatamente.',sentError.message);
     continue;
    }
    log.info('Notificação enviada.',{id:row.id,event:row.event_type,tool:row.tool});
   }catch(error){
    if(delivered){
     log.error('Mensagem enviada; falha posterior não será convertida em nova tentativa automática.',error.message);
     continue;
    }
