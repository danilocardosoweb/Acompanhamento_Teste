...
async function parseCorrections(bytes,current){
 ...
 if(correctionIndex>=0){
  ...
  const rows=source.map((row,index)=>{const lot=number(row[info.lot]),parsed=splitTool(row[info.tool]),correction=text(row[correctionIndex]),byLot=notes.filter(note=>number(note.payload?.Lote)===lot),byTool=notes.filter(note=>key(note.tool)===parsed.tool&&number(note.sequence)===parsed.sequence);let candidates=byLot.length?byLot:byTool,matchedBy=byLot.length?'lote':'ferramenta e sequência';if(candidates.length>1){const exact=candidates.filter(note=>key(note.tool)===parsed.tool&&number(note.sequence)===parsed.sequence);if(exact.length)candidates=exact;}const match=candidates.length===1?candidates[0]:null,testMatch=tests.some(test=>key(test.tool)===parsed.tool&&number(test.sequence)===parsed.sequence),status=match?'matched':candidates.length>1?'ambiguous':testMatch?'new':'unmatched';return {kind:'correction',row:info.row+index+2,lot,systemTool:text(row[info.tool]),tool:parsed.tool,sequence:parsed.sequence,correction,correctionDate:text(row[field(info.cells,'DATA CORRECAO')]),payload:{Lote:lot,Ferramenta:text(row[info.tool]),'Correção Efetuada':correction},matchId:match?.id||(status==='new'?hashId(`${lot}|${text(row[info.tool])}|${index}`):null),matchedBy:match?matchedBy:status==='new'?'ferramenta e sequência':null,status};});
